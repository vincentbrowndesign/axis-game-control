...
Warmup progression is separate from global memory count. The chain key is `twinId + warmupId`. If no profile exists, use `LOCAL PLAYER` so warmups connect immediately instead of feeling broken.
